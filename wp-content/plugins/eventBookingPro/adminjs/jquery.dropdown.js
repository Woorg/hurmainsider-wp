	...
	$.DropDown.prototype = {
		...
		_transformSelect : function() {
			...
			this.$el.children( 'option' ).each( function() {
				...
				if( val !== -1 ) {
					optshtml += 
						classes !== undefined ? 
							'<li data-value="' + val + '" data-allowed="' + allowed + '" data-cost="' + cost + '"><span class="' + classes + '">' + label + '</span></li>' :
							'<li data-value="' + val + '" data-allowed="' + allowed + '" data-cost="' + cost + '"><span>' + label + '</span></li>';
				}
		...
		open : function() {
			...
			h=(self.size.height>40)?self.size.height:40;
				$( this ).css( {
					opacity : 1,
					top : self.options.rotated ? h + self.options.gutter : ( i + 1 ) * ( h + self.options.gutter ),
