(function($) {
  ...
  defaults = {
    ...
    inputs: {
      ...
      paginationActiveClass: 'disabled',
  ...
  function Queries(obj, settings) {
    ...
    this.run = function() {
      for (query in settings.dataset.queries) {
        if (settings.dataset.queries.hasOwnProperty(query)) {
          ...
          if (_this.functions[query] === undefined) {
            ...
            if (queryColumn) {
              _this.functions[query] = function(record, queryValue) {
                return record[query].data == queryValue;
              };
            } else {
          ...
          var cmplxQueries, cmplQueryArray,
            cmplyQueryList, cmplQueryType
          for (quer in queryList) {

            if (queryList[quer].indexOf("&") > -1) {
              currQuery[quer] = "complex";
              cmplxQueries = queryList[quer].split("&");
              cmplQueryType = []
              cmplyQueryList = []
              for (ind in cmplxQueries) {
                cmplQueryType[ind] = "search";
                cmplyQueryList[ind] = cmplxQueries[ind];
                for (c in operations) {
                  if (cmplxQueries[ind].indexOf(operations[c]) > -1) {
                    cmplQueryType[ind] = "operationSearch";
                    cmplyQueryList[ind] = [cmplxQueries[ind], operations[c]]
                    break;
                  }
                }
              }

              queryList[quer] = [cmplQueryType, cmplyQueryList]
            } else {
              currQuery[quer] = "search";
              for (c in operations) {
                if (queryList[quer].indexOf(operations[c]) > -1) {
                  currQuery[quer] = "operationSearch";
                  queryList[quer] = [queryList[quer], operations[c]]
                  break;
                }
              }
          ...
          settings.dataset.records = $.map(settings.dataset.records, function(record) {
            isOkay = false
            for (quer in queryList) {
              if (currQuery[quer] == "complex") {
                isOkay = true;
                for (qL in queryList[quer]) {

                  isOkay = isOkay && _this.functions[queryList[quer][0][qL]](record, queryList[quer][1][qL]);
                }
              } else
                isOkay = _this.functions[currQuery[quer]](record, queryList[quer]);
              if (isOkay)
                break;

            }
            return isOkay ? record : null;
          });
        }
      }
    ...
    this.functions = {
      operationSearch: function(record, queryValue) {
        var operation = queryValue[1]
        var queryArray = queryValue[0].split(operation);
        var whichAttr = queryArray[0].replace(/\s+/g, "")
        var targetVal = queryArray[1].replace(/^\s+|\s+$/g, '')

        if (record.hasOwnProperty(whichAttr)) {
          var attrValue = record[whichAttr];

          switch (operation) {
            case "=":
              if (attrValue.toLowerCase().indexOf(targetVal.toLowerCase()) > -1) {
                return true;
              } else if (parseFloat(attrValue) == parseFloat(targetVal)) {
                return true;
              }
              break;

            case ">":
              if (_isStrDate(targetVal) && _isStrDate(attrValue)) {
                return compareDates(attrValue, targetVal) === 1;
            ...
            case ">=":
              if (_isStrDate(targetVal) && _isStrDate(attrValue)) {
                return compareDates(attrValue, targetVal) >= 0;
              }
              break
        ...
        function _isStrDate(searchValue) {
          return searchValue.match(/\d{2,}[-\]\d+[-]\d+/g);
        }
        ...
        function compareDates(a, b) {
          ...
          return (
            isFinite(a = dateA.valueOf()) &&
            isFinite(b = DateB.valueOf()) ? (a > b) - (a < b) : NaN
          );
      ...
      search: function(record, queryValue) {
        var contains = 0,
          queryArray = queryValue.match(/\S+/g);
        queryLength = queryArray.length
        ...
        for (attr in record) {
          if (record.hasOwnProperty(attr)) {
            ...
            if (typeof(attrValue) === "string") {
              ...
              for (qV in queryArray) {
                if (attrValue.toLowerCase().indexOf(queryArray[qV].toLowerCase()) > -1) {
                  contains++;
                  queryArray.splice(qV, 1);
                }
  ...
  function InputsSearch(obj, settings) {
    ...
    this.create = function() {
      var $search = $('<input placeholder="Advanced Search.  E.g. ticket=regular  & quantity>2 | Amount>30" />', {
          type: 'search',
        ...
        $searchHelp = $('<a class="tip-below tooltip noFloat" data-tip="How to use Advanced Search" href="http://iplusstd.com/item/eventBookingPro/example/advanced-search/" target="_blank">?</a>'),
        $searchSpan = $('<span></span>', {
          id: 'dynatable-search-' + obj.element.id,
          'class': 'dynatable-search',
          text: ''
        }).append($search).append($searchHelp);
